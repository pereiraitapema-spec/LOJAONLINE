            ...
            payments: [
              {
                ...
                credit_card: orderData.payment_method === 'credit_card' ? {
                  card: {
                    ...
                    exp_month: parseInt(orderData.expiry?.includes('/') ? orderData.expiry.split('/')[0].trim() : (orderData.expiry?.substring(0, 2) || '0'), 10),
                    exp_year: parseInt('20' + (orderData.expiry?.includes('/') ? (orderData.expiry.split('/')[1]?.trim() || '0') : (orderData.expiry?.substring(2, 4) || '0')), 10),
                    cvv: orderData.cvv,
                    billing_address: {
                      line_1: orderData.shipping_address.street ? `${orderData.shipping_address.number}, ${orderData.shipping_address.street}, ${orderData.shipping_address.neighborhood}` : 'Rua Fictícia, 123, Centro',
                      zip_code: orderData.shipping_address.cep && orderData.shipping_address.cep.toUpperCase() !== 'BALCAO' ? orderData.shipping_address.cep.replace(/\D/g, '') : '01001000',
                ...
                google_pay: orderData.payment_method === 'google_pay' ? (() => {
                  ...
                  console.log('💳 [GATEWAY] Normalizando token para Pagar.me...');

                  // Se vier como string encapsulada, tenta parsear
                  let tokenObj = typeof gPayToken === 'string' ? JSON.parse(gPayToken) : gPayToken;
                  
                  // Google Pay ECv2 format
                  const signedMsgStr = tokenObj?.signedMessage || tokenObj?.paymentMethodToken?.signedMessage;
                  const signedMessage = typeof signedMsgStr === 'string' ? JSON.parse(signedMsgStr) : signedMsgStr;
                  ...
                  return {
                    ...
                    signature: tokenObj?.signature || tokenObj?.paymentMethodToken?.signature,
                    header: header,
                    merchant_id: config.merchant_id || '10000000'
                  };
                })() : undefined
              }
            ].filter(p => {
